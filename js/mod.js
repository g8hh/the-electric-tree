...
function getPointGen() {
	...
	if(hasUpgrade("m", 82)) gain = gain.times(tmp.m.upgrades[82].effect)
	if(hasUpgrade("w", 11)) gain = gain.times(tmp.w.upgrades[11].effect)
	if(hasUpgrade("p", 42)) gain = gain.times(tmp.p.upgrades[42].effect)
	if(hasUpgrade("sh", 23)) gain = gain.times(tmp.sh.upgrades[23].effect)
	...
	if(hasUpgrade("m", 83)) gain = gain.times(tmp.cap.effect.pow(0.1))
	if(inChallenge("w", 22)) {
...
function isEndgame() {
	return player.com.points.gte(14) && player.cap.points.gte(16)
}
