/*

 @name    : 锅巴汉化 - Web汉化插件
 @author  : 麦子、JAR、小蓝、好阳光的小锅巴
 @version : V0.6.1 - 2019-07-09
 @website : http://www.g8hh.com

*/

//1.汉化杂项
var cnItems = {
    _OTHER_: [],

    //未分类：
    'Save': '保存',
    'Export': '导出',
    'Import': '导入',
    'Settings': '设置',
    'Achievements': '成就',
    'Statistics': '统计',
    'Changelog': '更新日志',
    'Hotkeys': '快捷键',
    'ALL': '全部',
    'Default': '默认',
    'AUTO': '自动',
    'default': '默认',
    "points": "点数",
    "Reset for +": "重置得到 + ",
    "Currently": "当前",
    "Effect": "效果",
    "Cost": "成本",
    "Buy new equipment to make even more electricity": "购买新设备以产生更多电力",
    "Economic Impact": "经济影响",
    "Electricty boosts electricity gain ( Don't ask me how": "电力增加电力增益（不要问我多少",
    "Employees": "员工",
    "Energy Boost": "能源提升",
    "Equipment": "设备",
    "Experts": "专家",
    "Hire employees to make more electricity": "雇用员工以产生更多电力",
    "Hire experts to boost your production": "聘请专家来提高您的产量",
    "Main": "主界面",
    "Market Pressure Vol1": "市场压力 Vol1",
    "money": "钱",
    "Refuse to sell electricity until the price goes up ( This game is slowly becoming illegal": "拒绝卖电直到价格上涨（这个游戏正在慢慢变得非法",
    "Unlock a money effect, which boosts electricty gain": "解锁金钱影响，提高电力收益",
    "Upgrades": "升级",
    "batteries": "电池",
    "Bigger Batteries": "更大的电池",
    "Buy bigger batteries": "购买更大的电池",
    "Batteries boost the money effect exponent": "电池推动金钱效应指数",
    "Battery Power": "电池力量",
    "Buying battery upgrades doesn't decrease your battery amount": "购买电池升级并不会减少您的电池电量",
    "Gain 100% of money on reset every second": "重置时每秒获得 100% 的金钱",
    "Keep money upgrades on reset": "重置时保留金钱升级",
    "Milestone Gotten!": "获得里程碑！",
    "Milestones": "里程碑",
    "Cost Decrease": "成本降低",
    "Batteries last longer and stock more electricity": "电池持续时间更长，储存更多电量",
    "Battery Extender": "电池扩展器",
    "Cheaper Batteries": "更便宜的电池",
    "Hire Managers": "招聘经理",
    "Hire managers to boost Employees effect": "聘请经理以提升员工效应",
    "Hire Technicians": "聘请技术员",
    "Hire technicians to boost battery effect": "聘请技术人员提高电池效果",
    "Market Pressure Vol2": "市场压力 Vol2",
    "Money effect divides battery cost at a reduced rate": "金钱效应以较低的比率划分电池成本",
    "More Upgrades": "更多升级",
    "Softcaps": "软上限",
    "Threaten to sell your electricity elsewhere if prices don't go up ( Illegal maneuvers : the return": "如果价格不上涨，则威胁要在其他地方出售电力（非法操作：回报）",
    "Unlock 3 battery upgrades": "解锁 3 次电池升级",
    "More Effects": "更多效果",
    "Cost Decrease Part 2": "成本降低第 2 部分",
    "Boost battery effect based on batteries": "基于电池的提升电池效果",
    "Battery effect reduces battery cost even more": "电池效应进一步降低电池成本",
    "Unlock the worker layer": "解锁工作层",
    "Workers": "工人",
    "Electricians": "电工",
    "Unlock a worker effect": "解锁工人效果",
    "Why Not Another Effect ?": "为什么不是另一种效果？",
    "workers": "工人",
    "Workers increase electricity gain": "工人增加电力增益",
    "Better Effect": "电池效果",
    "Triple the worker effect exponent": "三倍工人效应指数",
    "Base Booster": "基础助推器",
    "Batteries boost battery base": "电池提升电池基础",
    "Batteries boost worker gain": "电池提升工人增益",
    "Batteries cost less based on money": "电池成本更低，基于金钱",
    "Challenging": "挑战",
    "Challenges": "挑战",
    "Even More Upgrades": "更多升级",
    "Exit Early": "提前退出",
    "Finish": "完成",
    "Hardwork+": "勤奋+",
    "Hardwork": "勤奋",
    "Hardwork is more powerful and has a higher cap": "努力工作更强大，拥有更高的上限",
    "Low Demand": "需求降低",
    "Lowering Costs": "降低成本",
    "Money boosts electricity gain": "金钱增加电力收益",
    "There is a low demand of electricity, making the money gain exponent divide by 2": "电力需求低，使得货币收益指数除以2",
    "Completed": "已完成",
    "Goal:": "目标:",
    "Reward": "奖励",
    "Batteries reset nothing": "电池什么都不重置",
    "Gain 100% of worker gain on reset every second": "每秒重置时获得 100% 的工人收益",
    "Hire Workers": "雇用工人",
    "Money boosts worker gain": "金钱促进工人收益",
    "Unlock another 3 battery upgrades": "解锁另外 3 次电池升级",
    "Unlock the first worker challenge": "解锁第一个工人挑战",
    "Worker Boost": "工人提升",
    "Workers boost battery effect": "工人提升电池效应",
    "Keep Upgrading": "不断升级",
    "Unlock yet another 5 battery upgrades": "解锁另外 5 个电池升级",
    "Batteries need maitenance, hire more workers to take care of them": "电池需要维护，雇佣更多的工人来照顾它们",
    "Maintenance": "维护",
    "New Horizons": "新视野",
    "Overcharge": "过度充电",
    "Overcharge effect also raises worker effect to the 15th power": "过度充电效果还将工人效果提高到 15 次方",
    "Overcharge your batteries, which cubes their effect": "过度充电你的电池，这会增加它们的效果",
    "Overcharge+": "过度充电+",
    "Raise Hardwork cap to 2.5": "将勤奋上限提高到 2.5",
    "Unlock a new layer": "解锁一个新层",
    "Hardwork++": "勤奋++",
    "More Challenges": "更多挑战",
    "Unlock the second worker challenge": "解锁第二个工人挑战",
    "Start": "开始",
    "Permanently unlock the buyable": "永久解锁可购买",
    "Power Outage": "电力中断",
    "There has been a power outage, which removes the battery effect, but you unlocked a buyable as replacement": "发生了停电，消除了电池效应，但您解锁了可购买的替代品",
    "Buyables": "可购买",
    "Electric Powering": "电力供应",
    "Buyable Boost": "可购买提升",
    "First money buyable's effect also reduces battery cost": "第一个金钱可购买的效果也降低了电池成本",
    "Halves the first money buyable scaling": "将第一个钱可购买的缩放比例减半",
    "Lower Scaling": "降低缩放比例",
    "Battery Booster": "电池助推器",
    "Productive Workers": "生产工人",
    "Your workers are more productive, raising their effect to the 25th power": "您的员工生产力更高，将他们的影响提高到 25 次方",
    "coal power plants": "煤炭发电厂",
    "coal power plants": "煤炭发电厂",
    "Hire more workers to take care of your coal power plants": "雇用更多工人来照顾您的燃煤电厂",
    "That Coal Ain't Burning Itself": "煤不会自己燃烧",
    "Coal power plant resets nothing": "燃煤电厂什么都不重置",
    "Coal power plant's effect also boosts worker gain": "燃煤电厂的影响也提高了工人的收益",
    "Keep battery/worker milestones and worker challenges on reset": "保留电池/工人里程碑和工人挑战重置",
    "More Power Plants = More Jobs": "更多的发电厂 = 更多的工作",
    "Unlock auto battery prestige and battery buy max": "解锁自动电池声望和电池购买最大值",
    "Maybe Ecology Is Future ?": "也许生态是未来？",
    "More Corruption = Cheaper": "更多的腐败=更便宜",
    "Need More Effects": "需要更多效果",
    "Need More Power": "需要更多电力",
    "Political Boost": "政治提升",
    "Push corrupt politicians effect hardcap to 1.75": "将腐败政客的影响硬上限推至 1.75",
    "Pushing A Little Bit Further": "进一步推动一点",
    "Raise corrucpt politicians effect to the 1.25th power and raise the hardcap to 2": "将腐败政治家效应提高到 1.25 次方并将硬上限提高到 2",
    "Raise corrupt government effect to the 10th power": "将腐败政府效果提升至 10 次方",
    "Raise electricity gain to the 1.001 th power": "将电力增益提高到 1.001 次方",
    "Really ?": "真的吗 ？",
    "Remove the 5th softcap on the corrupt politician effect": "移除腐败政客效应的第 5 个软上限",
    "Remove the 6th softcap on the corrupt politician effect and make the 17th inflation upgrade better": "去除第 6 个软上限对腐败政客的影响，让第 17 个通胀升级更好",
    "Remove the first softcap of the corrupt politician effect": "去除腐败政客效应的第一个软上限",
    "Remove the second softcap on corrupt politician effect": "取消对腐败政客影响的第二个软上限",
    "Remove the useless softcap on inflation gain and raise inflation gain to the 2500th power": "删除无用的通胀收益软上限，将通胀收益提高到 2500 次方",
    "Self-Corruption": "自我腐败",
    "Shared Corruption": "共同腐败",
    "Softcapped": "软上限",
    "Softcapped ) The End Of The Trilogy": "软上限）三部曲的结局",
    "Softcapped ) The Sequel": "软上限）续集",
    "Spread corruption to the entire world with your corrupt governments": "用你的腐败政府将腐败传播到全世界",
    "Square corrupt politician effect": "平方腐败政客效应",
    "The first sofcap of the corrupt politician effect starts later": "腐败政客效应的第一个软上限开始较晚",
    "Unlock another battery buyable": "解锁另一个电池可购买",
    "Unlock another corrupt politician effect": "解锁另一个腐败政客效应",
    "Unlock another layer": "解锁另一个层",
    "Unlock new corrupt boost upgrades and corrupt boosts are 1% cheaper": "解锁新的腐败提升升级和腐败提升便宜 1%",
    "Unlock solar power plant challenges": "解锁太阳能发电厂的挑战",
    "Upgrade Stuff Before The Next Layer": "在下一层之前升级东西",
    "Unlock the fourth nuclear challenge": "解锁第四次核挑战",
    "Who Cares About Pollution ?": "谁在乎污染？",
    "Who Cares About Radioactivity ?": "Who Cares About Radioactivity ?",
    "Why ?": "为森么 ?",
    "Why must inflation stop ?": "为什么通货膨胀必须停止？",
    "Zimbabwe": "津巴布韦",
    "Dark Energy": "暗能量",
    "Cube Inflated effect": "立方膨胀效果",
    "eee1.000e10 inflation": "eee1.000e10 通货膨胀",
    "Effects, Effects, Effects...": "效果，效果，效果...",
    "Elecrticity boosts inflation generation speed": "电力提高通货膨胀的产生速度",
    "Extra Corruption": "额外的腐败",
    "F150,000,001 inflation": "F150,000,001 通货膨胀",
    "Finally, A Boost": "最后，一个提升",
    "Gain Control": "增益控制",
    "Generation": "世代",
    "Generation also boosts corrupt politician gain": "世代也助长了腐败政客的收益",
    "Generation doesn't remove corrupt politicians anymore and unlock 3 corrupt politician upgrades and 3 corrupt government upgrades": "世代不再清除腐败政客并解锁 3 个腐败政客升级和 3 个腐败政府升级",
    "Germany": "德国",
    "GET RID OF THEM": "摆脱它们",
    "Global demand on electricity rises, the government reduces by 1 the nuclear power plant cost exponent to satisfy the demand": "全球电力需求上升，政府将核电站成本指数降低1以满足需求",
    "Go Further": "走得更远",
    "Got Some Corruption With You ?": "你有腐败吗？",
    "Hell Remastered": "地狱重制",
    "HOW MANY SOFTCAPS DO YOU NEED ?": "您需要多少个软上限？",
    "Hyperinflated Boost": "过度膨胀的助推器",
    "Hyperinflation": "恶性通货膨胀",
    "I Hate Hardcaps": "我讨厌硬上限",
    "I Just Don't Care Anymore": "我只是不在乎了",
    "I n f l a t e": "膨胀",
    "Inflated": "膨胀",
    "Inflated+": "膨胀+",
    "Inflated++": "膨胀++",
    "Inflated Corruption": "膨胀的腐败",
    "Inflated Governments": "膨胀的政府",
    "Inflated Politics": "膨胀的政治",
    "inflation": "通货膨胀",
    "Inflation boosts battery amount in the battery effect formula": "电池效应公式中的通货膨胀提升电池量",
    "Inflation boosts corrupt government gain": "通货膨胀提升腐败政府获利",
    "Inflation boosts corrupt governments again": "通货膨胀再次提升腐败政府",
    "Inflation boosts corrupt politician and corrupt government gain": "通货膨胀提升腐败政客和腐败政府获利",
    "Inflation boosts electricity gain": "通货膨胀提升电力收益",
    "Inflation boosts infation again": "通胀再次提升通胀",
    "Inflation boosts inflation gain": "通胀提升通胀收益",
    "Inflation boosts its generation speed": "通货膨胀提升其生成速度",
    "Inflation go brrr": "通货膨胀到brrr",
    "Insert Title Here": "在此处插入标题",
    "Live TV Fight, Uhh I Mean Debate": "直播电视打架，呃，我是说辩论",
    "Lose 1% of your corrupt politicians every second, but they boost corrupt government gain": "每秒失去 1% 的腐败政客，但他们会增加腐败政府的收益",
    "Make the Hyperinflation formula better and enter the i n f l a t e d era": "使恶性通货膨胀公式更好，进入 通胀 时代",
    "Make the softcap on inflation gain 250x less powerful": "使通胀软上限收益降低 250 倍",
    "And Your Money Is Adding F150,000,000% To that Amount Every Second": "你的钱每秒增加 F150,000,000%",
    "Annilihate The Softcaps": "消灭软上限",
    "Another Boost ?": "另一个提升？",
    "Another Self-Boost I Guess": "另一个自我提升我猜",
    "Back To Hell": "回到地狱",
    "Base Destroyer": "基地毁灭者",
    "Battery Powering": "电池供电",
    "Big Ol' Boost": "大的提升",
    "Boost": "提升",
    "Boost all upgrades on the row above based on inflation": "根据通货膨胀提高上一行的所有升级",
    "Boost Inflated++ and Zimbabwe based on money": "基于金钱提升 膨胀++ 和津巴布韦",
    "Boost Inflated++ based on money": "基于金钱提升 膨胀++",
    "Boost inflation gain by a factor 5": "将通胀收益提高 5 倍",
    "Boosting And Boosting Again": "提升和再次提升",
    "CAP": "上限",
    "Capitalists and communists bases are 10% weaker": "资本主义者和共产主义者的基础弱了 10%",
    "Corrupt Corruption": "贪污腐败",
    "Corrupt goverments reduces corrupt boost cost": "腐败政府降低腐败成本",
    "corrupt governments": "腐败的政府",
    "Corrupt governments boost corrupt politician gain and remove 4th softcap on corrupt politician effect": "腐败政府提高腐败政客的收益并取消对腐败政客影响的第四次软上限",
    "Corrupt governments boost inflation gain": "腐败政府提升通胀收益",
    "Corrupt governments boost share gain": "腐败的政府促进了股票增长",
    "Corrupt governments boost their own gain": "腐败的政府增加了自己的收益",
    "Corrupt governments reduce taxes, making solar power plants cheaper": "腐败的政府减税，使太阳能发电厂更便宜",
    "Corrupt Hell": "腐败地狱",
    "corrupt politicians": "腐败的政客",
    "Corrupt politician effect is cubed": "腐败的政客效应是立方的",
    "Corrupt politicians and governments start producing corruption": "腐败的政客和政府开始制造腐败",
    "Corrupt politicians boost electricity gain": "腐败政客提高电力收益",
    "Corrupt politicians boost share gain": "腐败的政客推动了股票增长",
    "Corrupt politicians corrupt other politicians": "腐败的政客腐败其他政客",
    "Corrupt politicians reduce coal power plant cost exponent": "腐败政客降低燃煤电厂成本指数",
    "Corrupt politicians reduce nuclear power plant cost exponent": "腐败政客降低核电站成本指数",
    "Corrupt politicians reduce nuclear power plant cost": "腐败政客降低核电站成本",
    "Corruption-19": "腐败-19",
    "Wall Street": "华尔街",
    "You have suffered to get here, rest and take this boost. Battery cost is divided by money": "来到这里，休息并接受这种提升，你已经受了苦。 电池成本除以金钱",
    "Upgrades+": "升级+",
    "Upgrade Boost": "升级提升",
    "Unlock shares": "解锁股票",
    "Unlock anther layer. Might become hellish again. Also multiply electricity gain by 1e2000": "解锁另一个层。 可能又要变成地狱了。 还将电力增益乘以 1e2000",
    "Unlock a third worker challenge": "解锁第三个工人挑战",
    "Unlock a nuclear power plant buyable ( Don't worry, the buyable won't reset your progress :)": "解锁核电站可购买（别担心，可购买不会重置您的进度:)",
    "Unlock a battery buyable": "解锁电池可购买",
    "Unlock 9 new upgrades": "解锁 9 项新升级",
    "The previous upgrade also boosts share gain but stronger": "之前的升级也促进了股票增长但更强",
    "The 4th One": "第四个",
    "Take It Easy": "放轻松",
    "Solar Boost": "太阳能提升",
    "Softcapped ) 4th Of The Name": "软上限 ) 名字的第 4 个",
    "Savings": "存储",
    "Rest": "休息",
    "Remove the 3rd softcap on corrupt politician effect": "删除对腐败政客影响的第三个软上限",
    "Reassemble": "重装",
    "Reduce the solar power plant cost exponent by 0.005": "将太阳能发电厂成本指数降低 0.005",
    "Pushing A Litte Bit Further Again": "再次推动一点点",
    "Pay corrupt poiticians to reduce the first battery buyable cost scaling": "付钱给腐败的政客以减少第一块电池的可购买成本缩放",
    "Now That's What I Call Being Rich": "现在这就是我所说的富有",
    "Multiply by 25000 the amount of effective Buyable Boost levels": "乘以 25000 有效可购买提升等级的数量",
    "More Investments = More Profits": "更多投资 = 更多利润",
    "Money effect no longer divides costs at a reduced rate": "货币效应不再以较低的比率分配成本",
    "Money effect also divides coal power plant's cost at a reduced rate": "货币效应也降低了燃煤电厂的成本",
    "Money boosts the corrupt politician effect": "金钱提升腐败政客效应",
    "Money boosts corrupt politician gain": "金钱提升腐败政客获利",
    "Gain 10x more shares": "获得 10 倍以上的股票",
    "Form governments with your corrupt politicians": "与腐败的政客组成政府",
    "Extra Money": "额外的钱",
    "Exponent Booster": "指数助推器",
    "Even More Investments = Even More Profits": "更多的投资=更多的利润",
    "Electricity gain is raised to the 1.025th power": "电力增益提高到 1.025 次方",
    "Electric Powering is raised to the 100,000th power": "电力供应 提升至 100,000 次方",
    "Electric Investment": "电力投资",
    "Electric Powering 2nd Generation": "第二代电力",
    "Cost Reducer": "成本降低器",
    "Corruption : The Return": "腐败:回归",
    "Corrupt Batteries": "电池损坏",
    "Cooling Down": "冷却",
    "Coal Mine": "煤矿",
    "Capitalists' effect also boosts electricity gain but at a reduced rate": "资本家的影响也增加了电力收益，但速度有所下降",
    "Capitaslists' effect is stronger based on money": "资本家的效应是基于金钱的",
    "Calm Down": "冷静",
    "Buy More": "购买更多",
    "Build a cooling system for your coal power plants": "为您的燃煤电厂构建冷却系统",
    "Build a better cooling system for your nuclear power plants": "为您的核电站构建更好的冷却系统",
    "Better Cooling": "更好的冷却",
    "Another One": "另一个",
    "Another easy boost to recover. Nuclear power plant cost is divided by workers": "另一个容易恢复的推动力。 核电站成本除以工人",
    "And Another": "还有一个",
    "Younger Workers": "年轻工人",
    "Yes it is intended, bear with it": "是的，它是有意的，忍受它",
    "Workforce": "劳动力",
    "Workers divide the battery cost exponent": "工人除以电池成本指数",
    "Worker Power": "工人力量",
    "Worker effect is raised to the 2500th power": "工人效应提升到 2500 次方",
    "The 15th One": "第15个",
    "Solar Batteries": "太阳能电池",
    "Small Boost": "小提升",
    "Sheeper Batteries": "牧羊人电池",
    "Shares boost worker gain": "股票提升工人收益",
    "Shared Work": "共享工作",
    "Share gain is 1e6x greater": "股票收益是 1e6 倍",
    "Raise worker effect to the 20th power": "将工人效应提高到 20 次方",
    "Political Work": "政治工作",
    "Purchase Stuff": "购买东西",
    "Overcharge++": "超额充电++",
    "Ouch": "哎哟",
    "Network+": "网络+",
    "Network": "网络",
    "Lots Of Buyables": "大量可购买",
    "More Battery Power": "更多电池电量",
    "Last Minute Work": "最后一刻工作",
    "Hire some workers to build a better network between your coal power plants": "雇用一些工人在您的燃煤电厂之间建立更好的网络",
    "Gotta Go Faster": "必须走得更快",
    "Get Back To Work": "回去工作",
    "Flat Decrease": "扁平化减少",
    "Exponentiate electricity gain by 0.001 but multiply electricity gain by the 2nd money buyable effect": "将电力增益乘以 0.001，但将电力增益乘以第二个金钱可购买效果",
    "Exponent Crusher": "指数粉碎机",
    "Effect Booster": "效果助推器",
    "Each battery doubles worker gain": "每个电池使工人收益翻倍",
    "Divides the battery cost": "除以电池成本",
    "Divides battery cost by 1e695": "电池成本除以 1e695",
    "Deja Vu": "已经看过了",
    "Cube Network's effect": "立方网络效果",
    "Corrupt politicians boost worker gain": "腐败的政客提高工人的收益",
    "Combo": "组合",
    "CO2 Forever": "永远的二氧化碳",
    "Buying battery upgrades/buyables doesn't decrease your battery amount": "购买电池升级/可购买物品不会减少您的电池电量",
    "Better Buyables": "更好的可购买",
    "Battery effect is raised to the 1.1th power": "电池效果提升到1.1次方",
    "Battery overcharge cubes battery effect again": "电池过度充电再次使电池效果变立方",
    "Batteries Required": "需要电池",
    "Batteries reduce solar power plant cost": "电池降低太阳能发电厂成本",
    "Base Smasher": "基地破坏者",
    "Applies challenge 11 and 12 at the same time": "同时应用挑战 11 和 12",
    "Adds 0.015 to worker gain exponent": "将 0.015 添加到工人增益指数",
    "Add a 4th money buyable": "添加可购买的第 4 笔钱",
    "Add 1 to the money effect exponent": "货币效应指数加 1",
    "You Can't Buy Me :)-": "你不能买我 :)-",
    "Worthless Buyables": "无价值的可买",
    "Unlock Inflation": "解锁通货膨胀",
    "Unlock another nuclear buyable and add 0.02 to the base effect of the first": "解锁另一个核可购买，并在第一个的基础效果上增加 0.02",
    "Unlock an upgrade": "解锁一个升级",
    "Unlock another nuclear buyable": "解锁另一个核可购买",
    "Unlock a third nuclear challenge": "解锁第三个核挑战",
    "Unlock a second money buyable": "解锁第二个货币可购买",
    "Unlock a row of money upgrades": "解锁一排金钱升级",
    "Unlock a nuclear challenge": "解锁一个核挑战",
    "Unlock a multiplier to electricity gain that grows over time": "解锁随时间增长的电力增益乘数",
    "Unlock 2 new nuclear power plant buyables": "解锁 2 个新的核电站可购买",
    "Treatment Facilities": "处理设施",
    "Too Much Light": "光线太强",
    "The row 3 effects and side layers effects don't work anymore, and choose between 3 buffs and nerfs in the corrupt politician layer": "第 3 行效果和副作用层效果不再起作用，并在腐败政客层中的 3 个增益和削弱之间进行选择",
    "Square the solar power plant effect": "平方太阳能发电厂效应",
    "Solar power plants resets nothing": "太阳能发电厂不会重置任何东西",
    "Solar power plants require less batteries": "太阳能发电厂需要更少的电池",
    "solar power plants": "太阳能发电厂",
    "Solar power plant cost is brought to the 1.25th root": "太阳能发电厂成本被带到1.25次方根",
    "Solar Engineers": "太阳能工程师",
    "Softcaps Are Dumb": "软上限是愚蠢的",
    "Self Synergy": "自我协同",
    "Satan's Masterpiece": "撒旦的杰作",
    "Raise electricity gain to the 1.05 th power": "将电力增益提高到 1.05 次方",
    "Raise Curium-243's effect to the 8.50 th power": "将 锔-243 的效果提高到 8.50 次方",
    "Plutonium-239": "钚-239",
    "Nuclear Weapon": "核武器",
    "Nuclear power plants resets nothing": "核电站不会重置任何东西",
    "Nuclear power plants reduces battery exponent": "核电站降低电池指数",
    "nuclear power plants": "核电厂",
    "Nuclear power plant effect also applies to battery cost and unlock a third money buyable as well as another challenge": "核电站效应也适用于电池成本和解锁第三个可购买的钱以及另一个挑战",
    "Nuclear Engineers": "核工程师",
    "Nuclear Combo": "核组合",
    "Nuclear 5th Of The Name": "第五核的名字",
    "No Effects": "无效果",
    "No Can Do": "无能为力",
    "Multiply share gain based on nuclear buyables": "基于核可购买乘以股票收益",
    "Multiply corrupt politician gain based on nuclear power plants": "乘以腐败政客收益基于核电站",
    "Low Costs": "低成本",
    "Let's Get Even More Corrupt": "让我们变得更加腐败",
    "Keep battery upgrades on reset": "在重置时保持电池升级",
    "Hire engineers to take care of your nuclear power plants": "聘请工程师来照顾您的核电站",
    "Heavier Atoms": "重原子",
    "Gotta Go Fast": "得快点",
    "Exponent Reducer": "指数减速器",
    "Expensive Upgrades": "昂贵的升级",
    "Electricity gain drops over time until the challenge is completed": "电力增益会随着时间的推移而下降，直到挑战完成",
    "Divide the cost of solar power plants by 5": "将太阳能发电厂的成本除以 5",
    "Depends on the buff/nerf": "取决于增益/减益",
    "Curium-243": "锔-243",
    "Create waste treatment facilities, making nuclear power plants boost worker gain": "创建废物处理设施，使核电站提高工人收益",
    "Corrupt Synergy": "腐败的协同作用",
    "Corrupt politicians effect hardcap now becomes a softcap and quintuple corrupt politician gain": "腐败政客效果硬上限现在变成软上限和五倍腐败政客收益",
    "Coal power plants adds to their effect's base": "燃煤电厂增加了其效果的基础",
    "Coal is cheaper, thus your coal power plant effect is squared": "煤炭更便宜，因此您的燃煤电厂效应是平方的",
    "Cheap Fuel": "廉价燃料",
    "Californium-243": "锎-243",
    "Buying an upgrade divides global electricity production, and reduces their layer's production as well": "购买升级会除以全局电力生产，并降低其层的产量",
    "Buyables have no effect": "可购买没有影响",
    "Better Batteries": "更好的电池",
    "Berkelium-243": "锫-243",
    "Batteries Needed": "需要电池",
    "Anti-Corruption Politics": "反腐败政治",
    "And Yet Another": "又一个",
    "Already": "已经",
    "All layers effects are 1, but electricity gain is raised to the 15th power": "所有层效果均为1，但电力增益提高到15次方",
    "2nd Buyable": "第二可购买",
    "You think you deserve a reward for this ?": "你认为你应该为此得到奖励吗？",
    "corrupt boosts": "腐败提升",
    "Hire engineers to take care of your solar power plants": "聘请工程师来照顾您的太阳能发电厂",
    "Inflated Shares": "膨胀股票",
    "Inflation boosts share gain": "通货膨胀推动股票增长",
    "Inflationless": "无通胀",
    "Keep all row 5 upgrades": "保留所有第 5 行的 升级",
    "Keep all row 5 milestones": "保留所有第 5 行的 里程碑",
    "Keep all row 3 buyables": "保留所有第 3 行的 可购买",
    "Keep all row 3 challenges": "保留所有第 3 行的 挑战",
    "Keep all row 3 milestones and unlock aotu buy max for all row 3 currencies": "保留所有第 3 行的 里程碑并解锁所有第3行货币的自动购买最大值",
    "Keep all row 3 upgrades and battery buyables on reset": "保留所有第 3 行的 重置时的升级和电池可购买",
    "Keep share challenges": "保留股票挑战",
    "Keep share milestones": "保留股票里程碑",
    "Keep share upgrades": "保留股票升级",
    "Keep worker upgrades": "保留工人升级",
    "More Upgades": "更多升级",
    "Nuclear Boost": "核提升",
    "Nuclear Apocalypse": "核启示录",
    "You don't deserve any reward": "你不配得到任何奖励",
    "You can now gain both capitalists and communists": "你现在可以同时获得资本主义者和共产主义者",
    "What Do These Numbers Even Mean ?": "这些数字甚至意味着什么？",
    "Upgraded Shares": "升级股票",
    "Unlock another challenge": "解锁另一个挑战",
    "Unlock a whole lot of upgrades": "解锁一大堆升级",
    "Unlock a share effect": "解锁股票效果",
    "The title still gives you all the information you need": "标题仍然为您提供了您需要的所有信息",
    "The title gives you all the information you need": "标题为您提供了所需的所有信息",
    "The Other One": "另一个",
    "The three previous ones at the same time": "前三个同时",
    "Sharing Shares": "分享股票",
    "Sharing Even More Shares": "分享更多股票",
    "Shares boost share gain": "股票提升股票收益",
    "Shares boost electricity gain": "股票提升电力收益",
    "Shares boost corrupt goverment gain": "股票提升腐败政府获利",
    "shares": "股票",
    "Share Holder": "股东",
    "Satan's Masterpiece completions will remain because I'm not THAT heartless": "撒旦的杰作完成度将保留，因为我不是那么无情",
    "Reward ? Nah": "奖励？ 现在",
    "Reset all progress on rows 1-3 and corrupt politicians and governments ( Also Resets When Completing The Challenge": "重置第 1-3 行的所有进度和腐败的政客和政府（完成挑战时也会重置",
    "Powerless": "无力",
    "Real Boost": "真正的提升",
    "Hopeless": "绝望",
    "Heartless": "无情",
    "Gain 100% of shares on reset every second": "重置时每秒获得 100% 的股票",
    "Full Power": "全力",
    "Electric Chairs ( Oops made a totally not intentional spelling error": "电椅（哎呀，完全不是故意的拼写错误",
    "Do You Want To Suffer": "你想受苦吗",
    "Corruption boosts share gain": "腐败促进了股票收益",
    "Corrupt Shares": "腐败的股票",
    "Congratulations, you bought your first share, to reward you, shares boost corrupt politician gain and inflation generation speed": "恭喜你，你买了你的第一股，奖励你，股票提高腐败政客的收益和通货膨胀的产生速度",
    "Capitalists and communists both act as if chose them first and share gain is 10x greater": "资本主义者和共产主义者都表现得好像先选择了他们，分享收益是 10 倍",
    "Boost share gain based on money": "以金钱为基础提高股票收益",
    "Batteryless": "无电池",
    "Alright, you can get a bonus": "好吧，你可以得到奖金",
    "4th share challenges's reward is better": "第4次股票挑战奖励更好",
    "4th share challenge's reward is 20% better": "第 4 次股票挑战奖励提高 20%",
    "All nuclear buyables are 5x stronger": "所有核可购买都强 5 倍",
    "Battery base is 99.9% weaker and solar power plant base is 40% weaker": "电池基础弱 99.9%，太阳能发电厂基础弱 40%",
    "Black Friday": "黑色星期五",
    "Boost corrupt politician gain based on capitalists and capitalists upgrades": "基于资本家和资本家升级，提高腐败政客的收益",
    "Boost share gain based on them": "基于它们提高份额收益",
    "Boostin'": "提升",
    "Capital Boost": "资本提升",
    "Capitalism uses a stronger formula and it also boosts corrupt government gain": "资本主义使用更强大的公式，它也助长了腐败的政府收益",
    "Capitalism": "资本主义",
    "Capitalism+": "资本主义+",
    "capitalists": "资本主义者",
    "Capitalists boost share gain": "资本主义者推动股票收益",
    "Capitalists divides their own cost": "资本主义者除以自己成本",
    "Centuple the share gain exponent": "将股票收益指数乘以五倍",
    "communists": "共产主义者",
    "Communists are stronger together": "共产主义者团结起来更强大",
    "Compound Interest": "复利",
    "Corrupt boosts are 5% cheaper": "腐败提升便宜 5%",
    "Corrupt boosts and corrupt power are 50% stronger": "腐败加成和腐败力量增强 50%",
    "Corrupt boosts make capitalists cheaper": "腐败的提升使资本主义者更便宜",
    "Corrupt boosts make communists cheaper": "腐败的提升使共产主义者更便宜",
    "Corrupt boosts produce corrupt power, which increases the corrupt boost effect, but only when you are in the corrupt boosts tab": "腐败提升会产生腐败之力，这会增加腐败提升效果，但前提是你在腐败提升标签中",
    "Corrupt boosts reset nothing": "腐败的提升不会重置任何东西",
    "Corrupt Power": "腐败力量",
    "Corrupt power boosts its own generation": "腐败力量提升了自己的世代",
    "Corrupt power is stronger based on its own amount": "腐败力量更强基于自身数量",
    "Extra Capital": "额外资本",
    "I Need More Power": "我需要更多的力量",
    "I Sawed This Upgrade In Half": "我把这个升级一分为二",
    "I uhhh... whatever, this is an upgrade": "我呃...无论如何，这是一个升级",
    "Keep row 5 upgrades": "保留第 5 行 升级",
    "Keep row 5 milestones": "保留第 5 行 里程碑",
    "Keep share challlenges": "保留 股票 挑战",
    "Nuclear power plant base is 95% lower": "核电站基数降低 95%",
    "Row 5 layers reset nothing": "第 5 行的层不重置任何内容",
    "Share Your Shares": "分享您的股票",
    "Shared Workers": "持股工人",
    "The corrupt government effect gets better because why not and corrupt politicians boost share gain": "腐败的政府效果变得更好，因为为什么不和腐败的政客促进股份增长",
    "The Corruption Is Everywhere": "腐败无处不在",
    "This isn't a placeholder anymore ?": "这不再是占位符了吗？",
    "To Show You The Power Of Flex Tape": "向您展示软胶带的力量",
    "Unlock ??? new upgrades": "开锁 ？？？ 新升级",
    "Unlock more worker upgrades": "解锁更多工人升级",
    "Useful Corruption": "有用的腐败",
    "Well, I don't care": "好吧，我不在乎",
    "Workers boost share gain": "工人提高股票收益",
    "battery cost": "电池成本",
    "Press B to Battery Reset": "按 B 重置电池",
    "Press C to Coal Power Plant Reset": "按 C 键重置燃煤电厂",
    "Press H to Share Reset": "按 H 股票重置",
    "Press M to Money Reset": "按 M 重置金钱",
    "Press N to Nuclear Power Plant Reset": "按 N 重置核电站",
    "Press R to bulk buy all buyables of the layer you're in": "按 R 批量购买您所在层的所有可购买物品",
    "Press S to Solar Power Plant Reset": "按 S 重置太阳能发电厂",
    "Press SHIFT+L to Communist Reset": "按 SHIFT+L 重置共产主义者",
    "Press SHIFT+B to Corrupt Boost Reset": "按 SHIFT+B 腐败提升重置",
    "Press SHIFT+R to Capitalist Reset": "按 SHIFT+R 重置资本主义者",
    "Press U to buy all upgrades of the layer you're in": "按 U 购买您所在层的所有升级",
    "Press W to Worker Reset": "按 W 重置工人",
    "The Modding Tree": "The Modding Tree - 模组树",
    "v1.3: Still Can't Buy That God Damn Upgrade": "v1.3：还是买不起那该死的升级",
    "Bulk Buy All Battery Buyables": "批量购买所有电池",
    "Bulk Buy All Money Buyables": "批量购买所有金钱可购买",
    "Bulk Buy All Solar Buyables": "批量购买所有太阳能可购买",
    "Bulk Buy All Nuclear Buyables": "批量购买所有核能可购买",
    "Buy All Battery Upgrades": "购买所有电池升级",
    "Buy All Coal Upgrades": "购买所有煤炭升级",
    "Buy All Governments Upgrades": "购买所有政府升级",
    "Buy All Inflation Upgrades": "购买所有通胀升级",
    "Buy All Money Upgrades": "购买所有金钱升级",
    "Buy All Nuclear Upgrades": "购买所有核升级",
    "Buy All Politicians Upgrades": "购买所有政客升级",
    "Buy All Solar Upgrades": "购买所有太阳能升级",
    "Buy All Workers Upgrades": "购买所有工人升级",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "The Prestige Tree made by Jacorb and Aarex\n\t\t": "Jacorb 和 Aarex 制作的声望树\n\t\t",

    //树游戏
    'Loading...': '加载中...',
    'ALWAYS': '一直',
    'HARD RESET': '硬重置',
    'Export to clipboard': '导出到剪切板',
    'INCOMPLETE': '不完整',
    'HIDDEN': '隐藏',
    'AUTOMATION': '自动',
    'NEVER': '从不',
    'ON': '打开',
    'OFF': '关闭',
    'SHOWN': '显示',
    'Play Again': '再次游戏',
    'Keep Going': '继续',
    'The Modding Tree Discord': '模型树Discord',
    'You have': '你有',
    'It took you {{formatTime(player.timePlayed)}} to beat the game.': '花费了 {{formatTime(player.timePlayed)}} 时间去通关游戏.',
    'Congratulations! You have reached the end and beaten this game, but for now...': '恭喜你！ 您已经结束并通关了本游戏，但就目前而言...',
    'Main Prestige Tree server': '主声望树服务器',
    'Reach {{formatWhole(ENDGAME)}} to beat the game!': '达到 {{formatWhole(ENDGAME)}} 去通关游戏!',
    'Loading... (If this takes too long it means there was a serious error!)←': '正在加载...（如果时间太长，则表示存在严重错误！）←',
    'Main\n\t\t\t\tPrestige Tree server': '主\n\t\t\t\t声望树服务器',
    'The Modding Tree\n\t\t\t\t\t\t\tDiscord': '模型树\n\t\t\t\t\t\t\tDiscord',
    'Please check the Discord to see if there are new content updates!': '请检查 Discord 以查看是否有新的内容更新！',
    'aqua': '水色',
    'AUTOMATION, INCOMPLETE': '自动化，不完整',
    'LAST, AUTO, INCOMPLETE': '最后，自动，不完整',
    'NONE': '无',
    'P: Reset for': 'P: 重置获得',
    '': '',
    '': '',

}


//需处理的前缀
var cnPrefix = {
    "(-": "(-",
    "(+": "(+",
    "(": "(",
    "-": "-",
    "+": "+",
    " ": " ",
    ": ": "： ",
    "\n": "",
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": "",
    " ": "",
    //树游戏
    "Show Milestones: ": "显示里程碑：",
    "Autosave: ": "自动保存: ",
    "Offline Prod: ": "离线生产: ",
    "Completed Challenges: ": "完成的挑战: ",
    "High-Quality Tree: ": "高质量树贴图: ",
    "Offline Time: ": "离线时间: ",
    "Theme: ": "主题: ",
    "Anti-Epilepsy Mode: ": "抗癫痫模式：",
    "In-line Exponent: ": "直列指数：",
    "Single-Tab Mode: ": "单标签模式：",
    "Time Played: ": "已玩时长：",
    "Which Are Boosting Electricity Gain By ": "提高电力收益",
    "\t\t\t\t": "",
    "Show A Layer With Buttons To Buy Max For Mobile Players : ": "显示一个带有按钮的层，为手机玩家购买 最大：",
    "Which Are Boosting Corrupt Politician Gain By ": "提升腐败政客增益",
    "Which Are Raising Electricity Gain To The 25.01th power And Boosting Corrupt Politician Gain By ": "将电力增益提高到 25.01 次方并提高腐败政客的增益",
    "Reduces battery cost exponent by ": "降低电池成本指数",
    "Reduce the coal power plant cost exponent by ": "将燃煤电厂成本指数降低",
    "Multiply inflation generation speed by ": "将通货膨胀产生速度乘以",
    "Multiply battery effect base by ": "将电池效果基础乘以",
    "Boost all previous buyables by ": "通过以下方式提升所有以前的可购买",
    "Boost the first battery buyable's effect by ": "提升第一个电池可购买的效果",
    "Battery cost exponent is reduced by ": "电池成本指数减少了",
    "Battery cost base is divided by ": "电池成本基数除以",
    "Reduce the battery cost exponent by ": "降低电池成本指数",
    "Reduce solar power plant cost exponent by ": "减少太阳能发电厂成本指数",
    "Reduce nuclear power plant cost exponent by ": "减少核电站成本指数",
    "Reduce battery cost exponent by ": "降低电池成本指数",
    "Reduce all previous buyables' costs and divide nuclear power plant base by ": "降低所有以前可购买的成本并将核电站基数除以",
    "Corruption effect is reduced by ": "腐败效果降低",
    "Challenge completions : ": "挑战完成：",
    "Boosts corrupt politician gain by ": "提高腐败政客的收益",
    "Boost Corrupt Politician Gain By ": "提高腐败政客的收益",
    "Which Are Multiplying Worker Gain By ": "乘以工人收益",
    "Battery effect decreases ": "电池效果降低",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需处理的后缀
var cnPostfix = {
    ":": "：",
    "：": "：",
    ": ": "： ",
    "： ": "： ",
    "/s)": "/s)",
    "/s": "/s",
    ")": ")",
    "%": "%",
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": "",
    " ": " ",
    "\n": "",
    "\n\t\t\t": "\n\t\t\t",
    "\t\t\n\t\t": "\t\t\n\t\t",
    "\t\t\t\t": "\t\t\t\t",
    " to battery effect base": "到电池效果基数",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需排除的，正则匹配
var cnExcludeWhole = [
    /^(\d+)$/,
    /^([\d\.]+)e(\d+)$/,
    /^([\d\.]+)$/,
    /^e([\d\.]+)$/,
    /^([\d\.]+)s$/,
    /^([\d\.]+)x$/,
    /^x([\d\.]+)$/,
    /^([\d\.,]+)$/,
    /^([\d\.,]+)x$/,
    /^e([\d\.,]+)$/,
    /^e([\d\.,]+)x$/,
    /^x([\d\.,]+)$/,
    /^([\d\.]+)e([\d\.,]+)$/,
    /^x([\d\.]+)e([\d\.,]+)$/,
    /^([\d\.]+)e([\d\.,]+)x$/,
    /^[\u4E00-\u9FA5]+$/
];
var cnExcludePostfix = [
]

//正则替换，带数字的固定格式句子
//纯数字：(\d+)
//逗号：([\d\.,]+)
//小数点：([\d\.]+)
//原样输出的字段：(.+)
//换行加空格：\n(.+)
var cnRegReplace = new Map([
    [/^Which Are Raising Electricity Gain To The (.+)th power And Boosting Corrupt Politician Gain By (.+)$/, '将电力增益提高到 $1 次方并将腐败政客增益提高 $2 倍'],
    [/^Which Are Raising Electricity Gain To The (.+)th power$/, '将电力增益提高到 $1 次方'],
    [/^Which Are Multiplying Money Gain By (.+) And Multiplying Electricity Gain By (.+)$/, '货币收益乘以 $1 和电力收益乘以 $2'],
    [/^Which Are Raising Corrupt Goverment Gain And Their Effect To The (.+) Power$/, '提高腐败的政府收益及其对影响到 $1 次方'],
    [/^You have (.+) Corrupt Power, Making Corrupt Boosts (.+) More Efficient$/, '你有 $1 腐败能量，使腐败提升效率提高 $2 倍'],
    [/^Which Are Boosting Worker Gain By (.+) \( By Paying Them Below Minmum Wage Thanks To Your Corrupt Politicians$/, '这使工人的收益提高了 $1 倍（由于腐败的政客，他们支付的工资低于最低工资）'],
    [/^Reach (.+) undefined to unlock \(You have (.+) undefined$/, '达到 $1 undefined 解锁（你有 $2 undefined'],
    [/^Add (.+) to Nuclear Weapon's base$/, '增加 $1  到核武器的基数'],
    [/^(.+) And Divide Battery Cost By (.+)$/, '$1 并除以电池成本 $2'],
    [/^(.+) And Boosting Worker Gain By (.+)$/, '$1 并将工人收益提高 $2'],
    [/^Multiply Plutonium-239's effect by (.+)$/, '将 钚-239 的效果乘以 $1'],
    [/^Divides battery cost by (.+) and divides coal power plant cost by (.+)$/, '电池成本除以$1，煤电厂成本除以$2'],
    [/^Reduces battery base by (.+) and add (.+) free levels to Exponent Crusher$/, '将电池电量减少 $1 并为指数粉碎机添加 $2 个免费关卡'],
    [/^(.+) And Dividing Battery Cost By (.+)$/, '$1 并将电池成本除以 $2'],
    [/^(.+) And Dividing Battery\/Coal Power Plant Cost By (.+)$/, '$1 并将电池\/煤电厂成本除以 $2'],
    [/^Add (.+)  to the money effect exponent$/, '将 $1 添加到货币效应指数'],
    [/^And Your Money Is Adding (.+) To that Amount Every Second$/, '并且您的资金每秒增加 $1'],
    [/^Boosts electricity gain by (.+) and divides battery cost by (.+)$/, '将电力增益提高 $1 并将电池成本除以 $2'],
    [/^Boosts electricity gain by (.+)$/, '将电力增益提高 $1'],
    [/^Boost Electricty Gain By (.+)$/, '将电力增益提高 $1'],
    [/^You are gaining (.+) elves per second$/, '你每秒获得 $1 精灵'],
    [/^Reach (.+) workers to unlock \(You have (.+) workers$/, '达到 $1 工人即可解锁（您有 $2 工人'],
    [/^You have (.+) points$/, '你有 $1 点数'],
    [/^Battery effect \^(.+)$/, '电池效应 \^$1'],
    [/^(.+)x And Dividing Battery Cost By (.+)x$/, '$1x 并将电池成本除以 $2x'],
    [/^Next at (.+) KWh$/, '下一个在 $1 KWh'],
    [/^Next at (.+) money$/, '下一个在 $1 钱'],
    [/^Next at (.+) points$/, '下一个在 $1 点数'],
    [/^Reach (.+) money to unlock \(You have (.+) money$/, '达到 $1 钱解锁（你有 $2 钱'],
	[/^([\d\.]+)d ([\d\.]+)h ([\d\.]+)m ([\d\.]+)s"$/, '$1天 $2小时 $3分钟 $4秒'],
	[/^([\d\.]+)\% stronger$/, '$1\% 更强'],
	[/^([\d\.]+) batteries$/, '$1 电池'],
	[/^\/([\d\.]+)e([\d\.,]+) battery cost$/, '\/$1 电池成本'],
	[/^([\d\.]+) OOMs\/sec$/, '$1 OOMs\/秒'],
	[/^([\d\.,]+) OOMs\/sec$/, '$1 OOMs\/秒'],
	[/^([\d\.]+)\/sec$/, '$1\/秒'],
	[/^([\d\.,]+)\/sec$/, '$1\/秒'],
	[/^([\d\.]+)e([\d\.,]+)\/sec$/, '$1e$2\/秒'],
	[/^([\d\.]+)e([\d\.,]+) Workers$/, '$1e$2 工人'],
	[/^([\d\.]+)e([\d\.,]+) money$/, '$1e$2 钱'],
    [/^requires ([\d\.]+) more research points$/, '需要$1个研究点'],
    [/^([\d\.]+)e([\d\.,]+) points$/, '$1e$2 点数'],
    [/^([\d\.]+) elves$/, '$1 精灵'],
    [/^([\d\.]+) to money effect exponent$/, '$1 到金钱效果指数'],
    [/^([\d\.]+)e([\d\.,]+) workers$/, '$1e$2 工人'],
    [/^e([\d\.]+)e([\d\.,]+) Money$/, 'e$1e$2 钱'],
    [/^e([\d\.]+)e([\d\.,]+) money$/, 'e$1e$2 钱'],
    [/^e([\d\.]+)e([\d\.,]+) workers$/, 'e$1e$2 工人'],
    [/^e([\d\.]+)e([\d\.,]+)\/sec$/, 'e$1e$2\/秒'],
    [/^([\d\.]+)e([\d\.,]+) corrupt governments$/, '$1e$2 腐败的政府'],
    [/^([\d\.]+)e([\d\.,]+) elves$/, '$1e$2 精灵'],
    [/^([\d\.]+)e([\d\.,]+) shares$/, '$1e$2 股票'],
    [/^([\d\.]+)e([\d\.,]+) corrupt politicians$/, '$1e$2 腐败的政客'],
    [/^([\d\.]+)e([\d\.,]+) Shares$/, '$1e$2 股票'],
    [/^e([\d\.,]+) money$/, 'e$1 金钱'],
    [/^\*(.+) share gain$/, '\*$1 股票收益'],
    [/^\*(.+) to all upgrades effects in the row above$/, '\*$1 到 上一行的所有升级效果'],
    [/^\*(.+) to corrupt politician gain$/, '\*$1 到 腐败政客 收益'],
    [/^\*(.+) to corrupt government gain$/, '\*$1 到 腐败政府 收益'],
    [/^\*(.+) to corrupt goverment gain$/, '\*$1 到 腐败政府 收益'],
    [/^\*(.+) to Self Synergy effect$/, '\*$1 到 自我协同效应'],
    [/^\*(.+) to electicity gain$/, '\*$1 到 电力增益'],
    [/^\*(.+) worker gain$/, '\*$1 工人增益'],
    [/^\*(.+) to share gain$/, '\*$1 到 股票 增益'],
    [/^\*(.+) to corrupt politician gain and (.+) to inflation generation speed$/, '\*$1 腐败政客收益和 $2 通货膨胀产生速度'],
    [/^\*(.+) to inflation generation speed$/, '\*$1 到 通货膨胀产生速度'],
    [/^\*(.+) to inflation gain speed$/, '\*$1 到 通货膨胀增益速度'],
    [/^\*(.+) stronger$/, '\*$1 更强'],
    [/^(.+) capitalist cost$/, '$1 资本主义成本'],
    [/^(.+) communist cost$/, '$1 共产主义成本'],
    [/^(.+) to capitalist cost$/, '$1 到资本主义成本'],
    [/^(.+) to battery exponent$/, '$1 到 电池指数'],
    [/^(.+) to coal power plant effect base$/, '$1 到 煤电厂效应基数'],
    [/^(.+) to nuclear power plant cost exponent$/, '$1 到 核电站成本指数'],
    [/^(.+) to solar power plant cost$/, '$1 到 到太阳能发电厂成本'],
    [/^(.+) to nuclear power plant cost$/, '$1 到 核电站成本'],
    [/^(.+) to battery cost exponent$/, '$1 到 电池成本指数'],
    [/^(.+) battery cost$/, '$1 电池成本'],
    [/^(.+) to coal power plant cost exponent$/, '$1 到 燃煤电厂成本指数'],
    [/^([\d\.,]+) solar power plants$/, '$1 太阳能发电厂'],
    [/^(.+) cheaper corrupt boosts$/, '$1 更便宜的腐败提升'],
    [/^(.+) to battery cost$/, '$1 更电池成本'],
    [/^(.+) Nuclear Power Plants : Base Cost Exp (.+)$/, '$1 核电厂 : 基础成本指数 $2'],
    [/^(.+) Coal Power Plants : Base Cost Exp (.+)$/, '$1 煤电厂 : 基础成本指数 $2'],
    [/^(.+) Corrupt Boosts : Base Cost Exp (.+)$/, '$1 腐败提升 : 基础成本指数 $2'],
    [/^\^(.+) to electricity gain$/, '\^$1 到 电力增益'],
    [/^\^(.+) to inflation gain$/, '\^$1 到 通货膨胀增益'],
    [/^\^(.+) to battery amount in the effect formula$/, '\^$1 到 效果公式中的电池电量'],
    [/^\^(.+) to corrupt politician and corrupt governement gain$/, '\^$1 到 腐败的政客和腐败的政府增益'],
    [/^\^(.+) to Inflated\+\+ and Zimbabwe$/, '\^$1 到 到 膨胀\+\+ 和津巴布韦'],
    [/^\^(.+) to Inflated\+\+$/, '\^$1 到 到 膨胀\+\+'],
    [/^\^(.+) to Inflated effect$/, '\^$1 到 膨胀效应'],
    [/^\^(.+) to coal power plant effect$/, '\^$1 到 燃煤电厂的影响'],
    [/^\^(.+) to nuclear power plant effect$/, '\^$1 到 核电站的影响'],
    [/^\^(.+) to the corrupt politician effect \( before softcaps$/, '\^$1 到腐败政客的影响（在软上限之前'],
    [/^([\d\.,]+) Capitalist$/, '$1 资本主义者'],
    [/^([\d\.,]+) Capitalists$/, '$1 资本主义者'],
    [/^([\d\.,]+) Communist$/, '$1 共产主义者'],
    [/^([\d\.,]+) Communists$/, '$1 共产主义者'],
    [/^([\d\.,]+) Corrupt Boost$/, '$1 腐败提升'],
    [/^([\d\.,]+) Corrupt Boosts$/, '$1 腐败提升'],
    [/^([\d\.,]+) Nuclear Power Plants$/, '$1 核电厂'],
    [/^([\d\.,]+) Share$/, '$1 股票'],
    [/^([\d\.,]+) Shares$/, '$1 股票'],
    [/^([\d\.,]+) nuclear power plants$/, '$1 核电厂'],
    [/^([\d\.,]+) corrupt boosts$/, '$1 腐败提升'],
    [/^([\d\.,]+) Batteries$/, '$1 电池'],
    [/^([\d\.,]+) workers$/, '$1 工人'],
    [/^([\d\.,]+) money$/, '$1 钱'],
    [/^([\d\.,]+) batteries$/, '$1 电池'],
    [/^([\d\.,]+) capitalists$/, '$1 资本主义者'],
    [/^([\d\.,]+) communists$/, '$1 共产主义者'],
    [/^([\d\.,]+) corrupt politicians$/, '$1 腐败的政客'],
    [/^([\d\.,]+) elves$/, '$1 精灵'],
    [/^([\d\.,]+) Solar Power Plants$/, '$1 太阳能发电厂'],
    [/^([\d\.,]+) Coal Power Plants$/, '$1 煤炭发电厂'],
    [/^([\d\.,]+) coal power plants$/, '$1 煤炭发电厂'],
    [/^\*([\d\.]+) to money gain$/, '\*$1 到钱增益'],
    [/^\*(.+) to inflation gain$/, '\*$1 到 通胀收益'],
    [/^\*(.+) to worker gain$/, '\*$1 到工人增益'],
    [/^\*(.+) to corrupt power gain$/, '\*$1 到腐败权力增益'],
    [/^\*([\d\.]+) to electicity gain$/, '\*$1 到电力增益'],
    [/^\*(.+) to electricity gain$/, '\*$1 到电力增益'],
    [/^\*(.+) to elctricity gain$/, '\*$1 到电力增益'],
    [/^\^([\d\.]+) to battery effect$/, '\^$1 到电池效果'],
    [/^\^([\d\.]+) to Employees effect$/, '\^$1 到员工效果'],
    [/^\^([\d\.]+) battery effect$/, '\^$1 电池效果'],
    [/^([\d\.]+) Batteries : Base Cost Exp (.+)$/, '$1 电池 : 基础成本指数 $2'],
    [/^Cost : (.+) batteries$/, '成本：$1 电池'],
    [/^Cost: (.+) capitalists$/, '成本：$1 资本主义者'],
    [/^Cost: (.+) corrupt politicians$/, '成本：$1 腐败的政客'],
    [/^Cost: (.+) corrupt governments$/, '成本：$1 腐败的政府'],
    [/^Cost: (.+) inflation$/, '成本：$1 通货膨胀'],
    [/^Cost: (.+) coal power plants$/, '成本：$1 煤炭发电厂'],
    [/^Cost: (.+) communists$/, '成本：$1 共产主义者'],
    [/^Cost: (.+) corrupt boosts$/, '成本：$1 腐败提升'],
    [/^Cost: (.+) shares$/, '成本：$1 股票'],
    [/^Cost: (.+) solar power plants$/, '成本：$1 太阳能发电厂'],
    [/^Cost : (.+) solar power plants$/, '成本：$1 太阳能发电厂'],
    [/^Cost: (.+) nuclear power plants$/, '成本：$1 核电厂'],
    [/^Cost : (.+) nuclear power plants$/, '成本：$1 核电厂'],
    [/^Cost: (.+) batteries$/, '成本：$1 电池'],
    [/^Cost: (.+) money$/, '成本：$1 钱'],
    [/^Cost : (.+) money$/, '成本：$1 钱'],
    [/^Cost: (.+) workers$/, '成本：$1 工人'],
    [/^Cost: (.+) points$/, '成本：$1 点数'],
    [/^Req: (.+) \/ (.+) elves$/, '成本：$1 \/ $2 精灵'],
    [/^Req: (.+) \/ (.+) money$/, '成本：$1 \/ $2 钱'],
    [/^Req: (.+) \/ (.+) communists and capitalists$/, '成本：$1 \/ $2 共产主义者和资本主义者'],
    [/^Req: (.+) \/ (.+) workers$/, '成本：$1 \/ $2 工人'],
    [/^(.+) \/ (.+) batteries$/, '$1 \/ $2 电池'],
    [/^(.+) \/ (.+) shares$/, '$1 \/ $2 股票'],
    [/^(.+) \/ (.+) workers$/, '$1 \/ $2 工人'],
    [/^(.+) \/ (.+) money$/, '$1 \/ $2 钱'],
    [/^([\d\.]+)e([\d\.,]+) \/ ([\d\.]+)e([\d\.,]+) money$/, '$1e$2 \/ $3e$4 钱'],
    [/^Usages: (\d+)\/$/, '用途：$1\/'],
    [/^workers: (\d+)\/$/, '工人：$1\/'],

]);